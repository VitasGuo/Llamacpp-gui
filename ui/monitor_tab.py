...
class CompactMonitor(QWidget):
    ...
    加载模型/调参时无需切换到"性能监控"标签即可同时查看日志与系统负载。
    仅做展示：采样频率/历史/图表等完整能力仍在 MonitorTab（同一数据源）。
    """
    ...
    def set_focus_script(self, name):
        """聚焦脚本（脚本列表选中项）：t/s 与状态优先显示该服务。"""
        self._focus_name = name or ""

    @pyqtSlot(str, float)
    def update_tps(self, name, tps):
        """日志正则 t/s 通道（回退数据源，与 MonitorTab 同一信号）。"""
    ...
    def _update_gpus(self, gpus):
        if not gpus:
            # 与 MonitorTab 同理：仅判 parent 会在"消失→恢复"循环后不再恢复占位符
            if not self._gpu_placeholder.parent() or self._gpu_placeholder.isHidden():
