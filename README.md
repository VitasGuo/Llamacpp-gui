...
| 🔍 **模型搜索与下载** | 集成 ModelScope 与 hf-mirror.com，搜索模型、浏览 GGUF 文件、断点续传下载；**未搜索时默认显示"更新追踪"视图**——自动关注本地已装模型系列 + 搜索结果手动追踪，后台对比远端 `LastUpdatedTime` 有新版标黄，每行"查看文件"直达该模型文件列表下载；**本地模型文件管理**——"本地模型"入口列出模型目录下全部 .gguf（大小/修改时间/视觉投影标记），可永久删除（确认框列出文件与合计大小，连带清理启动脚本绑定并把 .bat 备份到 `data/scripts_replaced`、连带同目录配对 mmproj；模型正在运行时自动拦下） |
