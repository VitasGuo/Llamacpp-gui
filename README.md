...
| 🔍 **模型搜索与下载** | 集成 ModelScope 与 hf-mirror.com，搜索模型、浏览 GGUF 文件、断点续传下载；**未搜索时默认显示"更新追踪"视图**——自动关注本地已装模型系列 + 搜索结果手动追踪，后台对比远端 `LastUpdatedTime` 有新版标黄，每行"查看文件"直达该模型文件列表下载；**本地模型管理**——搜索栏右侧入口 toggle：默认「本地模型管理」/ 进栈索引 3 视图后同按钮位变「← 返回」，列出模型目录下全部 .gguf（大小/修改时间/视觉投影标记），可永久删除（确认框列出文件与合计大小，连带清理启动脚本绑定并把 .bat 备份到 `data/scripts_replaced`、连带同目录配对 mmproj；模型正在运行时自动拦下） |
...
│   ├── model_scanner.py    本地模型扫描（.gguf 列表：大小/修改时间/mmproj 标记）
│   ├── model_file_service.py 本地模型文件删除（逐项容错 + 配对 mmproj）
...
├── ui/                     界面层
│   ├── app.py              主窗口
│   ├── model_tab.py        模型搜索与下载标签页（含更新追踪默认视图 + 本地模型文件管理视图）
...
│   ├── dialogs/            对话框
│   └── workers/            后台工作线程（日志/状态轮询/搜索/版本安装/更新追踪/本地模型扫描与删除/Tailscale 探测/清理规划/进程清理）
